...
const WilayahRW = () => {
    ...
    const handlePrev = () => {
        setCurrentIndex((prevIndex) => (prevIndex === 0 ? rwData.length - 3 : prevIndex - 1));
    };

    // Fungsi untuk menggeser ke item berikutnya
    const handleNext = () => {
        setCurrentIndex((prevIndex) => (prevIndex === rwData.length - 3 ? 0 : prevIndex + 1));
    };
